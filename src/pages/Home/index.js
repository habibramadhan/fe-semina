...
const getAPICategories = async () => {
  ...
  const handleSubmit = async () => {
    try {
      if(type) {
        await axios.put('http://localhost:9000/api/v1/cms/categories/'+id, {name});
      } else {
        await axios.post('http://localhost:9000/api/v1/cms/categories', {name});
      }
      getAPICategories();
  ...
  const getOneCategories = async (id) => {
    const res = await axios.get(`http://localhost:9000/api/v1/cms/categories/${id}`);
